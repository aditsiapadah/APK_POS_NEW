...
<option value="id"
{{ old('bahasa',$setting->bahasa ?? 'id') == 'id' ? 'selected':'' }}>
...
<option value="IDR"
{{ old('mata_uang',$setting->mata_uang ?? 'IDR') == 'IDR' ? 'selected':'' }}>
...
@foreach([5,10,15,20,25,50] as $jumlah)


<option value="{{ $jumlah }}"

{{ old('per_page',$setting->per_page ?? 10) == $jumlah ? 'selected':'' }}>
